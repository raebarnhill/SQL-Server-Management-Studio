...
USE db_library
GO
CREATE PROCEDURE dbo.uspDueToday @BranchName VARCHAR(50)
AS 
SELECT LIBRARY_BRANCH.BranchID, LIBRARY_BRANCH.BranchName
FROM LIBRARY_BRANCH 
FULL OUTER JOIN BOOK_LOANS ON LIBRARY_BRANCH.BranchID = BOOK_LOANS.BranchID
FULL OUTER JOIN BORROWER ON BOOK_LOANS.CardNo = BORROWER.CardNo
GROUP BY 
	BORROWER.FullName, BORROWER.Borrower_Address, LIBRARY_BRANCH.BranchName, BOOK_LOANS.DateDue, LIBRARY_BRANCH.BranchID
HAVING 
	DateDue = CONVERT (varchar, getdate(), 1)
GO

EXEC dbo.uspDueToday @BranchName = 'Sharpstown'
...
GROUP BY 
	LIBRARY_BRANCH.BranchName, LIBRARY_BRANCH.BranchID
ORDER BY 
	COUNT(*) DESC
GO
...
FROM 
	BOOK_LOANS
FULL OUTER JOIN
	BORROWER ON BORROWER.CardNo = BOOK_LOANS.CardNo 
GROUP BY 
	BORROWER.FullName,BORROWER.Borrower_Address
HAVING 
	COUNT(*) > 5
GO

EXEC dbo.uspFivePlusLoans
